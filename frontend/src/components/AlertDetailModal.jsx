      ...
      >
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                <span className={`badge badge-${targetAlert.severity?.toLowerCase()}`} style={{ fontSize: '0.7rem' }}>
                  {targetAlert.severity} {t('alerts.title', 'ADVISORY')}
                </span>
                {isExpired ? (
                  <span className="badge badge-neutral" style={{ fontSize: '0.68rem' }}>{t('alerts.expired', 'EXPIRED / ARCHIVED')}</span>
                ) : (
                  <span className="badge badge-success" style={{ fontSize: '0.68rem' }}>{t('alerts.liveBroadcast', 'LIVE BROADCAST')}</span>
                )}
              </div>
        ...
        >
          <div style={{ background: 'rgba(15, 24, 44, 0.7)', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)' }}>
            <div style={{ color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{t('alerts.affectedLocation', 'Affected Location:')}</div>
          ...
          <div style={{ background: 'rgba(15, 24, 44, 0.7)', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)' }}>
            <div style={{ color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{t('alerts.hazardClassification', 'Hazard Classification:')}</div>
            <strong style={{ color: '#fbbf24' }}>{targetAlert.type || 'Civil Defense / Weather'}</strong>
          </div>

          <div style={{ background: 'rgba(15, 24, 44, 0.7)', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)' }}>
            <div style={{ color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{t('alerts.broadcastIssued', 'Issued Timestamp:')}</div>
          ...
          <div style={{ background: 'rgba(15, 24, 44, 0.7)', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)' }}>
            <div style={{ color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{t('alerts.autoExpiry', 'Expiration Status:')}</div>
            <strong style={{ color: isExpired ? '#94a3b8' : '#34d399' }}>
              {targetAlert.expiresAt ? new Date(targetAlert.expiresAt).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' }) : t('common.untilResolved', 'Until Resolved')}
            </strong>
          </div>
        </div>
        ...
        <button onClick={onClose} className="btn btn-primary" style={{ width: '100%', padding: '0.75rem' }}>
          {t('alerts.acknowledge', 'Acknowledge Advisory')}
        </button>
      </div>
    </div>
  );
